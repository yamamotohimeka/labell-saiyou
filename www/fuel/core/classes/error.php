...
class PhpErrorException extends \ErrorException
{
	...
	public function recover()
	{
		...
		if (static::$count <= \Config::get('errors.throttle', 10))
		{
			...
			{
				static::$count++;
				\Fuel\Core\Error::show_php_error(new \ErrorException($this->message, $this->code, 0, $this->file, $this->line));
			}
		}
		...
		{
			static::$count++;
			\Fuel\Core\Error::notice('Error throttling threshold was reached, no more full error reports are shown.', true);
		}
	}
}
...
class Error
{
	...
	public static function exception_handler(\Throwable $e)
	{
		...
		$is_production = class_exists('Fuel', false) ? (\Fuel::$env == \Fuel::PRODUCTION) : false;
		if ( ! $is_production)
		{
