...
register_shutdown_function(function ()
{
	...
	if (class_exists('Config', false) and \Config::get('session.native_emulation', false))
	{
...
set_exception_handler(function (\Throwable $e)
{
	...
	// PHP 7+ has a built-in \Error class; never use class_exists('Error') here.
	if ( ! class_exists('\\Fuel\\Core\\Error', false))
	{
...
function setup_autoloader()
{
	...
	// Enable global aliases (Fuel, Config, DB, etc.) and activate SPL autoloading.
	\Autoloader::add_core_namespace('Fuel\\Core');
	\Autoloader::register();
};
