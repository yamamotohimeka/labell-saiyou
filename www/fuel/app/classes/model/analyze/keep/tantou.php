...
class Analyze_Keep_Tantou extends \Model
{
    public static function get_keep_tantou($search = null)
    {
        $where = "";
        if(!empty($search)) {
            //面接日
            ...
            //面接担当
            if(!empty($search['select_staff_hidden'])){
                $whereList[] = "FIND_IN_SET(interview_staff, '$search[select_staff_hidden]')";
            }
...
//        $result["total_result"]["enrolled_count"] = $result["total_result"]["enter_count"] - $result["total_result"]["leaving_count"];
        ...
        // 継続率
        $result["total_result"]["total_per"] = $result["total_result"]["enrolled_count"] / $result["total_result"]["enter_count"] * 100;
//        if(($result["total_result"]["enter_count"] - $result["total_result"]["leaving_count"]) > 0) {
//            $result["total_result"]["total_per"] = ($result["total_result"]["enter_count"] - $result["total_result"]["leaving_count"]) / $result["total_result"]["enter_count"] * 100;
//        }
...
";
        $result["interview_staff"] = Common::get_data( $sql );

        foreach ($result["interview_staff"] as $key => $value) {

            $result["interview_result"][$value["interview_staff"]] = $value;

            $result["interview_result"][$value["interview_staff"]]["enrolled_count"] = $value["enter_count"] - $value["leaving_count"];

            //店ごとの入店率
            if($value["enter_count"] != 0){
                if(($value["enter_count"] - $value["leaving_count"]) > 0) {
//                    $result["detail_result"][$value["interview_staff"]]["total_per"] = ($value["enter_count"] - $value["leaving_count"]) / $value["enter_count"] * 100;
                    $result["detail_result"][$value["interview_staff"]]["total_per"] = $value["enrolled_count"] / $value["enter_count"] * 100;
                }
            }else{
                $result["detail_result"][$value["interview_staff"]]["total_per"] = 0;
            }
        }
