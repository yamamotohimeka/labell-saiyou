...
class Analyze_Recruit_Tantou extends \Model
{
    public static function get_recruit_tantou($search = null)
    {
        $where = "";
        if (!empty($search)) {
            //入店日
...
//        if ( $_SERVER["REMOTE_ADDR"] == "221.113.41.190" ) print_r($result["total_result"]);
        ...
        if (!empty($result["total_result"]["id"])) {
            ...
            if($result["total_result"]["enter_count"] > 0) {
...
";
            ...
            foreach ($result["interview_staff"] as $key => $value) {
                ...
                if ($value["enter_count"] != 0) {
                    if($value["enter_count"] > 0) {
//                        $result["detail_result"][$value["interview_staff"]]["total_per"] = $value["enter_count"] / $value["interview_count"] * 100;
                        $result["detail_result"][$value["interview_staff"]]["total_per"] = $value["enter_count"] / ( $value["interview_count"] - $value["notadopt"] ) * 100;
                    }
                } else {
