        ...
        <div class="send_wrap">
            {if isset($userData.group) && $userData.group == 1}
                <div class="{if $result.0.tentative_reserve_flg == 1}info_return{else}info_send{/if}">
                    <a href="{$base_url|default:'/'}inputdata/send_schdl/{$id}"><button type="submit"{if $result.0.tentative_reserve_flg == 1} disabled="disabled"{/if}>面接予定送信</button></a>
                </div>
            {/if}
            <div class="info_send">
...
{*                {/if}*}
                    {if !empty($result.0.belonging_store) AND !empty($result.0.interview_result) AND !empty($result.0.interview_staff) AND $result.0.check == 1 }
                        <a href="{$base_url|default:'/'}inputdata/send_rcrt/{$id}"><button type="submit">採用情報送信</button></a>
                    {else}
            ...
            <div class="info_return">
                <a href="{$base_url|default:'/'}inputdata/data/{$id}"><button type="submit">前のページに戻る <i class="fa fa-undo"></i></button></a>
            </div>
        </div>
    </section>
