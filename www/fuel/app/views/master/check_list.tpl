...
<article>
    ...
    <section>
        <div class="container table_cmn shop_wrap master_cmn_wrap">
            <h1 class="breadcrumb">
                <div class="btn_orange"><a href="/master/form/check">新規登録</a></div>
                <div class="btn_orange" style="width:auto">
                    <form action="/master/list/check" method="post" accept-charset="utf-8">
                        <input type="submit" name="submit_sort" value="並び替えを反映する" class="master_sort_btn" />
                        <input type="hidden" id="sort_num" name="sort" value="" />
                        <input type="hidden" name="sort_master" value="master_check" />
                    </form>
                </div>
                <span>確認状況の追加</span></h1>
            <table class="master_list_table">
                <tr>
                    <th class="sort_th">並び替え</th>
                    <th class="master_cmn_nm">確認状況</th>
                    <th class="master_cmn_nm" style="width: 80px;">背景色</th>
                ...
                {foreach from=$masterData name="masterData" item=value key=key}
                    <tr>
                        ...
                        <td class="edit"><a href="/master/form/check/{$value.id}">編集</a></td>
                        <td class="delete">
                            <form action="/master/list/check" method="post" id="master_delete_form" accept-charset="utf-8">
