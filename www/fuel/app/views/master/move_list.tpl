...
<article>
    ...
    <section>
        <div class="container table_cmn shop_wrap master_cmn_wrap">
            <h1 class="breadcrumb">
                <div class="btn_orange"><a href="/master/form/move">新規登録</a></div>
                <div class="btn_orange" style="width:auto">
                    <form action="/master/list/move" method="post" accept-charset="utf-8">
                        <input type="submit" name="submit_sort" value="並び替えを反映する" class="master_sort_btn" />
                        <input type="hidden" id="sort_num" name="sort" value="" />
                        <input type="hidden" name="sort_master" value="master_move" />
                    </form>
                </div>
                <span>出戻・移籍 etcの追加</span></h1>
            <table class="master_list_table">
                <tr>
                    <th class="sort_th">並び替え</th>
                    <th class="master_cmn_nm">出戻・移籍 etc</th>
                ...
                {foreach from=$masterData name="masterData" item=value key=key}
                    <tr>
                        ...
                        <td class="edit"><a href="/master/form/move/{$value.id}">編集</a></td>
                        <td class="delete">
                            <form action="/master/list/move" method="post" id="master_delete_form" accept-charset="utf-8">
